...
from chikoo import app, config, db, lang, userbot
...
async def _stats(_, m: types.Message):
    ...
    if str(pic).endswith((".mp4", ".gif", ".webm", ".mkv")):
        sent = await m.reply_video(video=pic, caption=m.lang["stats_fetching"])
    else:
        sent = await m.reply_photo(photo=pic, caption=m.lang["stats_fetching"])
